...
class Spreadsheet extends Component
{
    ...
    private $batchInfo;
    /**
     * @var array[] dimension options for all rows
     * can be overwrite with column dimensionOptions
     */
    ...
    public function render()
    {
        ...
        if(!empty($this->pageSettings))
            foreach ($this->pageSettings as $name => $value) {
                $method = 'set' . ucfirst($name);
                $pageSetup = $document->getActiveSheet()->getPageSetup();
                call_user_func([$pageSetup, $method], $value);
            }
    ...
    protected function renderBody($models, $keys, &$modelIndex)
    {
        foreach ($models as $index => $model) {
            ...
            foreach ($this->columns as $column) {
                ...
                $is_incolumn = key_exists ( $colomnKey, $this->has_extended );
                if ($is_incolumn !== false) {
                    $style = $column->renderDataCellStyle($model, $key, $modelIndex);
                    $value = $column->renderDataCellContent($model, $key, $modelIndex);
                    $inner_columns = unserialize( $value );
                    for($i = 0; $i < count ( $inner_columns ); ++ $i) {
                        $this->renderCell($columnIndex . $this->rowIndex, $inner_columns[$i], $style);
                        $columnIndex++;
                    }
                } else {
                    /* @var $column Column */
                    ...
                    if(!empty($this->colorize_if)){
                        $value = $column->renderDataCellContent($model, $key, $modelIndex);
                        if($value == $this->colorize_if['value']){
                            $last_col = range('A','Z')[$this->getTotalColumn() - 1];
                            $this->applyCellStyle($columnIndex.($this->rowIndex).':'.($last_col).($this->rowIndex), [
                                'font' => ['bold' => true],
                                'alignment' => ['wrapText' => true],
                                'fill' => [
                                    'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
                                    'startColor' => ['argb' => $this->colorize_if['color']]
                                ],
                            ]);
                            $this->striped_ignore[] = $this->rowIndex;
                        }
                    }
    ...
    private function getTotalColumn() {
        if ($this->has_extended != null) {
            $total = count ( $this->columns );
            foreach ( $this->has_extended as $extra_columns ) {
                $total += count ( $extra_columns ) - 1;
            }
            return $total;
        } else {
    ...
    protected function renderHeader()
    {
        ...
        if (empty($this->headerColumnUnions)) {
            ...
            foreach ($this->columns as $key => $column) {
                /* @var $column Column */
                $is_incolumn = key_exists ($columnKey, $this->has_extended);
                if ($is_incolumn !== false) {
                    for($i=0; $i < count($this->has_extended[$columnKey]); ++$i){
                        $this->renderCell($columnIndex.$this->rowIndex, $this->has_extended[$columnKey][$i]);
                        $columnIndex++;
                    }
                } else {
            ...
            foreach ($this->headerColumnUnions as $columnUnion) {
                ...
                while (true) {
                    ...
                    if ($is_incolumn !== false) {
                        for($i=0; $i < count($this->has_extended[$columnKey]); ++$i){
                            $this->renderCell($columnIndex.($this->rowIndex + 1), $this->has_extended[$columnKey][$i]);

                            if($i < count($this->has_extended[$columnKey]) - 1)
                                $columnIndex++;
                        }
        ...
        $last_col = range('A','Z')[$this->getTotalColumn() - 1];
        $this->applyCellStyle('A2:'.($last_col).($this->rowIndex-1), [
            'font' => ['bold' => true],
            'alignment' => ['wrapText' => true],
            'fill' => [
                'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
        ...
        $sheet->freezePane ( 'A' . ($this->rowIndex) );
        $sheet->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, $this->rowIndex);
    }
    ...
    protected function applyColumnOptions()
    {
        ...
        foreach ($this->columns as $column) {
            /* @var $column Column */
            if (!empty($this->dimensionOptions) || !empty($column->dimensionOptions)) {

                $column->dimensionOptions = array_merge($this->dimensionOptions,$column->dimensionOptions);
