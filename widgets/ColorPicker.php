...
class ColorPicker extends InputWidget
{
    ...
    public function run()
    {
        ...
        if ($this->addon && !$this->inline) {
            $addon = Html::tag('span', $this->addon, ['class' => 'input-group-text']);
            $add_con = Html::tag('div', $addon, ['class' => 'input-group-append']);
            $input = strtr($this->template, ['{input}' => $input, '{addon}' => $add_con]);
