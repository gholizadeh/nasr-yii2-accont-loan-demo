...
use app\assets\MonthPickerAsset;

/**
 * DatePicker renders a `monthpicker` jQuery UI widget.
 *
 * For example to use the monthpicker with a [[\yii\base\Model|model]]:
 *
 * ```php
 ...
 *
 */
class MonthPicker extends InputWidget
{
    /**
    ...
    public $dateFormat;
    /**
     * @var array the bind function to assign to month picker.
     */
    public $bind;
    /**
    ...
    public function run()
    {
        ...
        if ($language !== 'en-US') {
            ...
            $view->registerJs("jQuery('#{$containerID}').monthpicker($.extend({}, $.datepicker.regional['{$language}'], $options));");
        } else {
            $this->registerClientOptions('monthpicker', $containerID);
        }

        $this->registerClientEvents('monthpicker', $containerID);
        $this->registerClientBinds('monthpicker', $containerID);
        JuiAsset::register($this->getView());
        MonthPickerAsset::register($this->getView());
    }

    protected function registerClientBinds($name, $id)
    {
        if (!empty($this->bind)) {
            $js = [];
            foreach ($this->bind as $item => $function) {
                $js[]="jQuery('#$id').$name().bind('$item',$function);";
            }
