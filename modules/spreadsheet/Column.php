...
class Column extends BaseObject
{
    ...
    public function renderDataCellStyle($model, $key, $index){
        ...
        if($this->grid->striped && !in_array($this->grid->rowIndex, $this->grid->striped_ignore) && !isset($style['fill']) && ($index % 2 == 0)){
            $style['fill'] = [
        ...
        if(!empty($this->grid->contentOptions)){
            foreach ($this->grid->contentOptions as $key => $value) {
                if(!isset($style[$key]))
                    $style[$key] = $value;
            }
        }
    ...
    {
        $style = $this->renderDataCellStyle($model, $key, $index);
